...
// UART I/O port base addresses
enum class port_base : uint16_t { COM1 = 0x3F8, COM2 = 0x2F8, COM3 = 0x3EF, COM4 = 0x2EF };

// UART register offsets
constexpr uint16_t data_port_offset(uint16_t base) {
    return base + 0;
}
constexpr uint16_t interrupt_enable_port_offset(uint16_t base) {
    return base + 1;
}
constexpr uint16_t fifo_command_port_offset(uint16_t base) {
    return base + 2;
}
constexpr uint16_t line_command_port_offset(uint16_t base) {
    return base + 3;
}
constexpr uint16_t modem_command_port_offset(uint16_t base) {
    return base + 4;
}
constexpr uint16_t line_status_port_offset(uint16_t base) {
    return base + 5;
}

// Line Control Register (LCR) flags
enum class line_control_flags : uint8_t {
    ENABLE_DLAB = 0x80,                  // Enable Divisor Latch Access
    EIGHT_BITS_NO_PARITY_ONE_STOP = 0x03 // 8 bits, no parity, 1 stop bit
};

// FIFO Control Register (FCR) flags
enum class fifo_control_flags : uint8_t {
    ENABLE_FIFO = 0x01,         // Enable FIFO
    CLEAR_RECEIVE_FIFO = 0x02,  // Clear Receive FIFO
    CLEAR_TRANSMIT_FIFO = 0x04, // Clear Transmit FIFO
    TRIGGER_14_BYTES = 0xC0     // Set trigger level to 14 bytes
};

// Modem Control Register (MCR) flags
enum class modem_control_flags : uint8_t {
    RTS_DSR = 0x03, // Ready to Send (RTS), Data Set Ready (DSR)
    OUT2 = 0x08     // OUT2 bit, required to enable UART interrupts
};

// Line Status Register (LSR) flags
enum class line_status_flags : uint8_t {
    TRANSMIT_EMPTY = 0x20, // Transmitter Holding Register Empty
    DATA_READY = 0x01      // Data Ready
};

// Baud rate divisors (assuming 1.8432 MHz clock)
enum class baud_rate_divisor : uint8_t {
    BAUD_115200 = 0x01, // 115200 baud
    BAUD_57600 = 0x02,  // 57600 baud
    BAUD_38400 = 0x03,  // 38400 baud
    BAUD_19200 = 0x06,  // 19200 baud
    BAUD_9600 = 0x0C,   // 9600 baud
    BAUD_4800 = 0x18,   // 4800 baud
    BAUD_2400 = 0x30,   // 2400 baud
 ...
 * @param baud_divisor The divisor value for the desired baud rate.
 */
void init_port(uint16_t port, baud_rate_divisor baud_divisor = baud_rate_divisor::BAUD_9600);
